...
// GALERİ SEÇİCİ FONKSİYONLARI (Boyut Sorunu Çözüldü)
function openGalleryPicker() {
    ...
    if(myPhotos.length === 0) {
        // Grid yapısını bozmaması için yazıyı 3 sütuna da yayıyoruz (grid-column: span 3)
    ...
    for(let i = myPhotos.length - 1; i >= 0; i--) {
        let p = myPhotos[i];
        // ÇÖZÜM: Fotoğrafları kesinlikle kare ve küçük yapacak inline CSS eklendi
        container.innerHTML += `<div style="width: 100%; aspect-ratio: 1/1; background-image:url('${p.photo_data}'); background-size:cover; background-position:center; border-radius:5px; cursor:pointer; border:1px solid #444;" onclick="selectAdPhoto('${p.photo_data}')"></div>`;
    }
}
